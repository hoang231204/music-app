...
export const create = async (req: Request, res: Response) => {
    try {
        res.json({ code: 200, message: "Form tạo ca sĩ mới" });
    }
    catch (err) {
        console.error(err);
        res.status(500).json({ code: 500, message: "Lỗi server" });
...
export const createPost = async (req: Request, res: Response) => {
    try{
        ...
        res.json({ code: 200, message: "Tạo ca sĩ thành công" });
    }
    catch(err){
        console.error(err);
        res.status(500).json({ code: 500, message: "Lỗi server" });
...
export const edit = async (req: Request, res: Response) => {
    try{
        ...
        if (!singer) {
            return res.status(404).json({ code: 404, message: "Không tìm thấy ca sĩ" });
        }
        res.json({ code: 200, data: singer });
    }
    catch(err){
        console.error(err);
        res.status(500).json({ code: 500, message: "Lỗi server" });
