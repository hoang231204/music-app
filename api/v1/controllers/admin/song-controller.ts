...
export const create = async (req: Request, res: Response) => {
    try {
        ...
        res.json({ code: 200, data: { singers, topics } });
    }
    catch (err) {
        console.error(err);
        res.status(500).json({ code: 500, message: "Lỗi server" });
...
export const createPost = async (req: Request, res: Response) => {
    try{
        ...
        await newSong.save();
        res.json({ code: 200, message: "Tạo bài hát thành công" });
    }
    catch (err) {
        console.error(err);
        res.status(500).json({ code: 500, message: "Lỗi server" });
...
export const edit = async (req: Request, res: Response) => {
    try {
        ...
        const singers = await Singer.find({ deleted: false }).select("fullname");
        const topics = await Topic.find({ deleted: false }).select("title");
        res.json({ code: 200, data: { song, singers, topics } });
    }
    catch (err) {
        console.error(err);
        res.status(500).json({ code: 500, message: "Lỗi server" });
